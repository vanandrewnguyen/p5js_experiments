...
This script uses arrays to cycle through each respective point in order to emulate a worm-like movement pattern.
The mock 3D effect is made through calculating distance between points and drawing an ellipse.
...
// Declare Worm Variables
let xx = []; let yy = [];
let circleNum = 24;
// Declare random coord Variables
let tarX = 0;
let tarY = 0;
let tarTimer = 90;
let tarTimerInt = 0;
...
function draw() {
    ...
    //text(str(mouseSpd),50,50); <- draw the speed of the mouse (debug only)
    ...
    if (mouseSpd == 0) {
        ...
        if (tarTimerInt > tarTimer) {
            tarTimerInt = 0; 
            tarTimer = random(45, 90);
            tarX = random(64, cWidth - 64); 
            tarY = random(64, cHeight - 64);
        }
    } else {
        tarX = mouseX; 
        tarY = mouseY;
